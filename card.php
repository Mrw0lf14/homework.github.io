	...
	<div class="content">
			...
			<?php
				...
				$download = "<a title='download $ndownloads times' href=download.php?id=$id align='center'>download</a>";
			 	if($owner == $_SESSION['username']){
			 		$moddel = "<a href = delete.php?id=$id>Delete</a>";
			 	}
				echo "<img src=$pict>"
			?>
			<div class="head">
		...
		<?php  
			...
			while($row = mysqli_fetch_row($res)){
				...
				$comname = $row[2];
				$comlike = $row[3];
				if($comname==$_SESSION['username'] || $_SESSION['username']=="admin"){
					$comdelete = "<a class='comdel' href='delete.php?id=$id&name=$comname'>delete</a>";
				}
				echo "
				<div class='coments'>
					<h3>$comname</h3>$comdelete
					<div class='text'>
					$comtext
					</div>
...
<?php
	if (isset($_POST['comment'])){
		...
		$query = "SELECT * FROM comments WHERE own = '$comname' AND id = $id";
		$res = mysqli_query($link ,$query); 
		$row = mysqli_fetch_row($res);
		if ($row){
